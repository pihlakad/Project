...
namespace Logic {
    public static class Round {
        ...
        internal static decimal decimalCoeficient(sbyte percision) {
            return Convert.ToDecimal(Math.Pow(10.0, percision));
        }
        ...
        public static decimal Off(decimal d, Rounding p) {
            switch (p.Strategy) {
                case Approximate.Up:return Up(d, p.Decimals);
                case Approximate.Down: return Down(d, p.Decimals);
                case Approximate.UpByStep: return UpByStep(d, p.Step);
                case Approximate.DownByStep: return DownByStep(d, p.Step);
                case Approximate.TowardsPositive: return TowardsPositive(d, p.Decimals);
                case Approximate.TowardsNegative: return TowardsNegative(d, p.Decimals);
                case Approximate.Common: return Off(d, p.Decimals, p.Digit);
                default: return Off(d, p.Decimals, p.Digit);
            }
        }
        public static double Off(double d, Rounding p) {
            switch (p.Strategy) {
                case Approximate.Up:return Up(d, p.Decimals);
                case Approximate.Down: return Down(d, p.Decimals);
                case Approximate.UpByStep: return UpByStep(d, p.Step);
